...
- **`index.html`** — formulário público que os convidados preenchem (acessado via
  ...
  Vinuta?" numa escala de **1 a 5**, representada por 5 taças de vinho.
...
- **`admin.html`** — painel interno (acessado via QR code / link 2), protegido por
  ...
  - Histograma "Votos por taça" (quantas pessoas deram cada nota de 1 a 5)
...
1. Começou como avaliação 1–5 taças (nota de degustação de vinho) → foi
   convertido para NPS 0–10 → o usuário pediu para voltar para 1–5, mas
   mantendo a pergunta em estilo NPS ("O quanto você recomendaria a
   Vinuta?"). É esse o formato final: pergunta de recomendação, escala 1–5.
